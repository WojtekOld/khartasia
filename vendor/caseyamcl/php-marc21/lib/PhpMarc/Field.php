...
Class Field {
    ...
    function __construct()
    {
        ...
        $this->is_control = (preg_match("/^\d+$/", (string) $tagno) && $tagno < 10);
        if($this->is_control) {
            $this->data = array_shift($args);
        } else {
            foreach (["ind1", "ind2"] as $indcode) {
                $indicator = array_shift($args);
                if(!preg_match("/^[0-9A-Za-z ]$/", (string) $indicator)) {
                    if($indicator != "") {
    ...
    {
        if (strtolower($obj::class) == Field::class) {
            ...
            $this->_croak(sprintf("Argument must be Field-object, but was '%s'", $obj::class));
        }
    }
    ...
    {
        if($this->is_control) {
            return $this->data.static::getEndOfField();
        } else {
            $subfields = [];
            foreach ($this->subfields as $subfield => $value) {
                $subfields[] = static::getSubFieldIndicator().$subfield.$value;
            }
            return $this->ind1.$this->ind2.implode("", $subfields).static::getEndOfField();
        }
    }
    ...
    function string($fields = "")
    {
        $matches = [];
        if($fields) {
            for($i=0; $i<strlen((string) $fields); $i++) {
