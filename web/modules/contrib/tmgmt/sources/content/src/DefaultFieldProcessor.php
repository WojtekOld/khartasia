  ...
  public function extractTranslatableData(FieldItemListInterface $field) {
    ...
    foreach ($field as $delta => $field_item) {
      ...
      foreach ($field_item->getProperties(TRUE) as $property_key => $property) {
        ...
        if ($property_definition instanceof DataReferenceDefinitionInterface && $property_definition->getTargetDefinition()->getDataType() == 'entity:file') {
          $target = $property->getValue();
          if ($target instanceof FileInterface) {

            $config = \Drupal::config('tmgmt.settings');
            if (in_array($target->getMimeType(), (array) $config->get('file_mimetypes'))) {
              $translate = TRUE;
        ...
        if (!($property instanceof PrimitiveInterface) || $property_definition->isComputed()) {
          continue;
        }
        ...
        if (
          ...
          && is_numeric($schema['columns'][$property_key]['length'] ?? NULL)
        ) {
          $data[$delta][$property_key]['#max_length'] = $schema['columns'][$property_key]['length'];
        }
    ...
    foreach (Element::children($field_data) as $delta) {
      ...
      foreach (Element::children($field_item) as $property) {
        ...
        if ((isset($property_data['#translation']['#text']) || isset($property_data['#translation']['#file']))  && $property_data['#translate']) {
          ...
          $field->offsetGet($delta)->set($property, $property_data['#translation'][$translation_key]);
        }
