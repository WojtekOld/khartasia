...
class ContentEntitySourceUntranslatableTargetsUnitTest extends ContentEntityTestBase {
  ...
  public function testEmbeddedReferencesUntranslatableTargets() {
    ...
    // Add a field to the untranslatable composite, with translatable storage
    // but untranslatable instance; it must not be extracted.
    ...
    $referenced_entities[2]->set('untranslatable_field', 'Do not translate me');
   ...
   //  $this->assertEquals('Delta #0', $data['t_composite'][0]['#label']);
    ...
    // On an untranslatable bundle, a field with
    // translatable storage but untranslatable instance is not extracted.
    $this->assertFalse(isset($data['t_composite'][0]['entity']['untranslatable_field']));
