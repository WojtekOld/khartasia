...
class ContentTmgmtEntitySourceUiTest extends TMGMTTestBase {
  ...
  function testNodeTranslateTabSingleCheckout() {
    $this->loginAsTranslator(array('translate any entity', 'create content translations', 'administer tmgmt'));
    ...
    $this->assertSession()->pageTextContains('Pending Translations');
    ...
    $this->assertSession()->pageTextContains((string) t('The translation for @title has been accepted as @target.', array('@title' => $node->getTitle(), '@target' => $translation->label())));
    ...
    $this->submitForm([], 'Validate HTML tags');
    ...
    $this->assertEquals('Translation Outdated', $xpath[0]->getAttribute('title'));
    ...
    // With purging enabled (the default), deleting a source removes its job
    // items and deletes the job once it is left empty.
    $this->config('tmgmt.settings')->set('purge_stale', TRUE)->save();
    $purged_node = $this->createTranslatableNode('page', 'en');
    $this->drupalGet('node/' . $purged_node->id() . '/translations');
    $this->submitForm(['languages[de]' => TRUE], 'Request translation');
    $this->submitForm([], 'Submit to provider');

    $item_storage = \Drupal::entityTypeManager()->getStorage('tmgmt_job_item');
    $properties = ['item_type' => 'node', 'item_id' => $purged_node->id()];
    $items = $item_storage->loadByProperties($properties);
    $purged_job_id = reset($items)->getJobId();

    // The job is listed on the jobs overview.
    $this->drupalGet('admin/tmgmt/jobs');
    $this->assertSession()->linkByHrefExists('admin/tmgmt/jobs/' . $purged_job_id);

    // Deleting the source purges its job item and the now empty job.
    $purged_node->delete();
    $this->assertEmpty($item_storage->loadByProperties($properties));
    $this->assertNull(Job::load($purged_job_id));

    // The purged job is gone from the overview.
    $this->drupalGet('admin/tmgmt/jobs');
    $this->assertSession()->linkByHrefNotExists('admin/tmgmt/jobs/' . $purged_job_id);

    // With purging disabled, a stale job item is kept when its source is
    // deleted, so the job can not be accepted and reports the missing entity.
    ...
    $this->submitForm([], 'Submit to provider');
    $this->assertSession()->pageTextContains('1 conflicting item has been dropped.');
  ...
  function testNodeTranslateTabMultipleCheckout() {
    ...
    // Verify that we are on the translate tab.
    $this->assertSession()->pageTextContains('3 jobs need to be checked out.');
  ...
  function testCommentTranslateTab() {
    ...
    $this->submitForm([], 'Submit to provider and continue');
    $this->assertSession()->pageTextContains($comment->getSubject());
    $this->submitForm([], 'Submit to provider');
  ...
  function testEmbeddedReferences() {
    ...
    $this->submitForm($checked_reference_fields, 'Save configuration');
  ...
  function testEntitySourcePreview() {
    ...
    $this->submitForm(['items[1]' => 1], 'Request translation');
    $this->submitForm(['target_language' => 'de', 'translator' => 'test_translator'], 'Submit to provider');
    $this->clickLink('reviewed');
    $this->clickLink('Preview');
    $this->assertSession()->pageTextContains('Preview of de(de-ch): Custom Block for German');

    // The preview guard for a missing source only applies when stale job items
    // are kept, so opt out of purging for this scenario.
    $this->config('tmgmt.settings')->set('purge_stale', FALSE)->save();
    ...
    $this->submitForm(['title|0|value[translation]' => 'test_translation'], 'Save');
    $this->assertSession()->pageTextContains('The translation has been saved successfully.');
    ...
    $this->assertSession()->linkNotExists('Preview');
  ...
  public function testSourceOverview() {
    ...
    $this->submitForm($edit, 'Check for continuous jobs');
    $this->assertSession()->pageTextContainsOnce("1 continuous job item has been created.");
    ...
    $this->submitForm($second_edit, 'Check for continuous jobs');
    $this->assertSession()->pageTextContainsOnce('None of the selected sources can be added to continuous jobs.');
    ...
    $this->submitForm($add_all_edit, 'Check for continuous jobs');
    $this->assertSession()->pageTextContainsOnce('2 continuous job items have been created.');
