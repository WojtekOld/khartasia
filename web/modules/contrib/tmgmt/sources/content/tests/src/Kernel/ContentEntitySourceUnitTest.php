...
class ContentEntitySourceUnitTest extends ContentEntityTestBase {
  ...
  protected $later_addition_label;
  ...
  public function setUp(): void {
    ...
    // Add a field with translatable storage but untranslatable instance; it
  ...
  public function testEntityTest() {
    ...
    $entity_test->untranslatable_field->appendItem([
      'value' => 'This field is not translatable at the instance level.',
      'format' => 'text_plain',
    ]);
    ...
    // On a translatable bundle, a field with translatable storage but
    // untranslatable instance is not extracted.
    $this->assertFalse(isset($data['untranslatable_field']));
  ...
  public function testNode() {
    ...
    $this->assertTrue($tracked_translation_insertions['node'][$node->id()]['de']);
  ...
  public function testContinuousJobItems() {
    ...
    $continuous_job_de = tmgmt_job_create('en', 'de', $account->id(), [
      'job_type' => Job::TYPE_CONTINUOUS,
      'translator' => $translator,
      'continuous_settings' => $continuous_settings,
    ]);
    $this->assertEquals(SAVED_NEW, $continuous_job_de->save());

    // Create a second continuous job for spanish.
    $continuous_job_es = tmgmt_job_create('en', 'es', $account->id(), [
      'job_type' => Job::TYPE_CONTINUOUS,
      'translator' => $translator,
      'continuous_settings' => $continuous_settings,
    ]);
    $this->assertEquals(SAVED_NEW, $continuous_job_es->save());
    ...
    $prevented_node->save();
    $this->assertEquals(0, count($continuous_job_de->getItems()));
    ...
    $continuous_job_de_items = $continuous_job_de->getItems();
    $continuous_job_de_item = reset($continuous_job_de_items);
    $this->assertEquals($node->label(), $continuous_job_de_item->label(), 'Continuous job item is automatically created for an english node.');
    $continuous_job_es_items = $continuous_job_es->getItems();
    $continuous_job_es_item = reset($continuous_job_es_items);
    $this->assertEquals($node->label(), $continuous_job_es_item->label(), 'Continuous job item is automatically created for an english node.');

    // Test that continuous job item is in state review.
    $this->assertEquals(JobItemInterface::STATE_REVIEW, $continuous_job_de_item->getState(), 'Translation for an english node is in state review.');
    $this->assertEquals(JobItemInterface::STATE_REVIEW, $continuous_job_es_item->getState(), 'Translation for an english node is in state review.');
    ...
    $this->assertEquals(1, count($continuous_job_de->getItems()), 'There are no new job items for an english node.');
    $this->assertEquals(1, count($continuous_job_es->getItems()), 'There are no new job items for an english node.');

    // Accept translation for an english node.
    $continuous_job_de_item->acceptTranslation();
    $continuous_job_es_item->acceptTranslation();

    // Test that the translation for an english node is created and saved.
    $node = Node::load($node->id());
    $translation_de = $node->getTranslation('de');
    $data_de = $continuous_job_de_item->getData();
    $this->assertEquals($data_de['title'][0]['value']['#translation']['#text'], $translation_de->label(), 'Translation for an english node has been saved correctly.');
    $this->assertEquals(JobItemInterface::STATE_ACCEPTED, $continuous_job_de_item->getState(), 'Translation for an english node has been accepted.');
    $translation_es = $node->getTranslation('es');
    $data_es = $continuous_job_es_item->getData();
    $this->assertEquals($data_es['title'][0]['value']['#translation']['#text'], $translation_es->label(), 'Translation for an english node has been saved correctly.');
    $this->assertEquals(JobItemInterface::STATE_ACCEPTED, $continuous_job_es_item->getState(), 'Translation for an english node has been accepted.');
    ...
    $this->assertEquals(1, count($continuous_job_de->getItems()), 'Continuous job item is not created for a german node.');

    // Create a spanish node.
    $spanish_node = Node::create([
      'title' => $this->randomMachineName(),
      'uid' => $account->id(),
      'type' => $type->id(),
      'langcode' => 'es',
    ...
    // Test that there is no new item for spanish node.
    $this->assertEquals(1, count($continuous_job_es->getItems()), 'Continuous job item is not created for a spanish node.');
    ...
    $this->assertEquals(1, count($continuous_job_de->getItems()), 'Continuous job item is not created for a second english node.');
    $this->assertEquals(1, count($continuous_job_es->getItems()), 'Continuous job item is not created for a second english node.');
    ...
    $this->assertEquals(1, count($continuous_job_de->getItems()), 'Continuous job item is not created for an updated english node.');
    $this->assertEquals(1, count($continuous_job_es->getItems()), 'Continuous job item is not created for an updated english node.');

    // Set the outdated flag to true.
    $translation_de = $node->getTranslation('de');
    $translation_de->content_translation_outdated->value = 1;
    $translation_de->save();
    $translation_es = $node->getTranslation('es');
    $translation_es->content_translation_outdated->value = 1;
    $translation_es->save();

    // Test that there are now two items for english node.
    $this->assertCount(2, $continuous_job_de->getItems(), 'Continuous job item is automatically created for an updated english node.');
    $this->assertCount(2, $continuous_job_es->getItems(), 'Continuous job item is automatically created for an updated english node.');

    $continuous_job_item_de_recent = $continuous_job_de->getMostRecentItem('content', $node->getEntityTypeId(), $node->id());
    $continuous_job_item_es_recent = $continuous_job_es->getMostRecentItem('content', $node->getEntityTypeId(), $node->id());

    // Set job item state to aborted.
    $continuous_job_item_de_recent->setState(JobItemInterface::STATE_ABORTED, NULL, [], 'status');
    $continuous_job_item_es_recent->setState(JobItemInterface::STATE_ABORTED, NULL, [], 'status');
    ...
    $this->assertEquals(3, count($continuous_job_de->getItems()), 'Continuous job item is automatically created for an updated english node.');
    $this->assertEquals(3, count($continuous_job_es->getItems()), 'Continuous job item is automatically created for an updated english node.');

    // Accept the most recent translation for both translations.
    $continuous_job_item_de_recent = $continuous_job_de->getMostRecentItem('content', $node->getEntityTypeId(), $node->id());
    $continuous_job_item_es_recent = $continuous_job_es->getMostRecentItem('content', $node->getEntityTypeId(), $node->id());
    $continuous_job_item_de_recent->acceptTranslation();
    $continuous_job_item_es_recent->acceptTranslation();

    // Re-load the node and check translations are now outdated.
    $node = \Drupal::entityTypeManager()->getStorage('node')->loadUnchanged($node->id());
    $de_metadata = $content_translation_manager->getTranslationMetadata($node->getTranslation('de'));
    $es_metadata = $content_translation_manager->getTranslationMetadata($node->getTranslation('es'));
    $this->assertFalse($de_metadata->isOutdated());
    $this->assertFalse($es_metadata->isOutdated());

    // There should only be 3 items still and no loop when there are multiple
    // jobs.
    // @see https://www.drupal.org/project/tmgmt/issues/3063378
    $this->assertEquals(3, count($continuous_job_de->getItems()), 'No continuous job item is automatically created when translation is accepted.');
    $this->assertEquals(3, count($continuous_job_es->getItems()), 'No continuous job item is automatically created when translation is accepted.');
