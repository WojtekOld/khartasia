...
use Drupal\tmgmt\Entity\Controller\JobItemAccessControlHandler;
...
use Drupal\tmgmt\Form\JobItemDeleteForm;
...
#[ContentEntityType(
  id: 'tmgmt_job_item',
  label: new TranslatableMarkup('Translation Job Item'),
  entity_keys: [
    'id' => 'tjiid',
    'uuid' => 'uuid',
  ],
  handlers: [
    'access' => JobItemAccessControlHandler::class,
    'form' => [
      'edit' => JobItemForm::class,
      'abort' => JObItemAbortForm::class,
      'delete' => JobItemDeleteForm::class,
    ],
    'list_builder' => JobItemListBuilder::class,
    'view_builder' => EntityViewBuilder::class,
    'views_data' => JobItemViewsData::class,
  ],
  links: [
    'canonical' => '/admin/tmgmt/items/{tmgmt_job_item}',
    'abort-form' => '/admin/tmgmt/items/{tmgmt_job_item}/abort',
...
class JobItem extends ContentEntityBase implements JobItemInterface {
  ...
  public function getFileCount() {
    return (int) $this->get('file_count')->value;
  }
  ...
  public function acceptTranslation() {
    if (!$this->isNeedsReview()) {
      $this->addMessage("Job item {$this->id()} status is not 'needs review'");
      return FALSE;
    }

    if (!$plugin = $this->getSourcePlugin()) {
      $this->addMessage("Invalid source plugin for job item {$this->id()}");
      return FALSE;
    }

    if (!$this->applyFieldLengthOverflowPolicy()) {
      return FALSE;
    }

    if (!$plugin->saveTranslation($this, $this->getJob()->getTargetLangcode())) {
      $this->addMessage("Unknown error while saving translation for job item {$this->id()}");
      return FALSE;
    }
  ...
  /**
   * Applies the configured policy for translations longer than their field.
   *
   * @return bool
   *   TRUE if acceptance may proceed, FALSE if it must stop for review.
   */
  protected function applyFieldLengthOverflowPolicy(): bool {
    /** @var \Drupal\tmgmt\Data $data_service */
    $data_service = \Drupal::service('tmgmt.data');
    $policy = \Drupal::config('tmgmt.settings')->get('field_length_overflow_policy');

    $valid = TRUE;
    $truncated = FALSE;
    foreach ($data_service->filterTranslatable($this->getData()) as $key => $item) {
      if (empty($item['#max_length']) || !isset($item['#translation']['#text'])) {
        continue;
      }
      ...
      if ($policy === 'truncate') {
        // Shorten the translation at a word boundary to fit the column.
        $this->updateData($key, ['#translation' => ['#text' => Unicode::truncate($text, $max_length, TRUE, TRUE)]]);
        $this->addMessage('The translation for %field was longer than the maximum of @max characters allowed and has been truncated.', [
          '%field' => $data_service->itemLabel($item),
          '@max' => $max_length,
        ], 'warning');
        $truncated = TRUE;
      }
      else {
        // Keep the item in review so the translation can be handled manually.
        $review_url = $this->toUrl('canonical', ['query' => ['destination' => $this->getJob()->toUrl()->toString()]])->toString();
        $this->addMessage('The translation for %field is longer than the maximum of @max characters allowed and cannot be accepted. Shorten it in the <a href=":review_url">review form</a>.', [
          '%field' => $data_service->itemLabel($item),
          '@max' => $max_length,
  ...
  protected function count(&$item) {
    if (!empty($item['#text']) || !empty($item['#file'])) {
      if (\Drupal::service('tmgmt.data')->filterDataWithFiles($item)) {
