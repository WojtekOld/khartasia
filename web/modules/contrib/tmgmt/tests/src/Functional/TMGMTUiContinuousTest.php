...
class TMGMTUiContinuousTest extends TMGMTTestBase {
  ...
  public function testContinuous() {
    ...
    $this->submitForm($edit_continuous_job, 'Save job');
    ...
    // The table order is nonpredictable because is ordered by the changed time,
    // which in test is almost every time the same. Target the link accurately.
    $row = $this->getSession()->getPage()->find('xpath', '//tr/td[normalize-space(text())="'. $continuous_job_label .'"]/..');
    $row->clickLink('Manage');
    ...
    $this->assertSession()->pageTextNotContains('Job type');
    ...
    $this->assertSession()->pageTextContains('No job items for the current selection.');
  }
