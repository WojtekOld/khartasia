...
class BibciteEntityTest extends BrowserTestBase {
  ...
  public function testSettingsLinksForm() {
    ...
    $page->uncheckField('edit-links-pubmedcentral-enabled');
    ...
    $page->checkField('edit-links-pubmedcentral-enabled');
    $page->pressButton('edit-submit');
    $this->assertSession()->statusCodeEquals(200);
  }
   ...
   */
  public function testReferenceTable() {
    $this->drupalLogin($this->user);

    $this->drupalGet('bibcite/reference/1');

    // Download citation table row exists.
    $this->assertSession()->elementExists('xpath', '//div[contains(@class, "bibcite-reference-table")]//tr[th[text() = "Download citation"]][td//a[text() = "Google Scholar"]]');
  }
