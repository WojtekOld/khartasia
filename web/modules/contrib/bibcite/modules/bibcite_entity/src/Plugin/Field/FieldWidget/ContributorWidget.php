...
class ContributorWidget extends EntityReferenceAutocompleteWidget implements ContainerFactoryPluginInterface {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    // Provide our own custom entity autocomplete validation so we can wrap it
    // in exception handling.
    $element['target_id']['#element_validate'] = [[get_class($this), 'validateValidContributorName']];
  ...
  public static function validateValidContributorName($element, FormStateInterface $form_state, array &$complete_form) {
    try {
      ...
      // usually does, but catch an exception thrown when a contributor is
      // auto-created from an invalid input.
      EntityAutocomplete::validateEntityAutocomplete($element, $form_state, $complete_form);
    }
    catch (NameParsingException) {
      $form_state->setError($element, t('Invalid name entry. Please enter a single name in the format "First [Middle] Last" or "Last, First [Middle]".'));
    }
  }
