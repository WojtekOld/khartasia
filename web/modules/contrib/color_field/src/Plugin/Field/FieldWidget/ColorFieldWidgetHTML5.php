  ...
  public function formMultipleElements(FieldItemListInterface $items, array &$form, FormStateInterface $form_state) {
    ...
    if (!$this->getSetting('show_extra') && $elements['#max_delta'] > 0) {
      ...
      if ($cardinality === FieldStorageDefinitionInterface::CARDINALITY_UNLIMITED) {
        // Skip suppression when the user just clicked "Add another item" for
        // this field. Core's addMoreAjax expects the new empty slot at
        // #max_delta to exist; removing it would produce a blank row.
        $triggering = $form_state->getTriggeringElement();
        if ($triggering && isset($elements['add_more']['#name']) &&
            $triggering['#name'] === $elements['add_more']['#name']) {
          return $elements;
        }
        // Remove the last numerically keyed entry only when it is genuinely
        // the extra empty slot added by core. After a remove operation the
        // slot may hold real data (a value shifted down from a higher delta
        // by deleteSubmit), in which case suppression would silently discard
        // a user-entered value. Check user input rather than #default_value,
        // since default values reflect the saved entity state and are always
        // empty for items that were added via AJAX but not yet saved.
        $field_input = NestedArray::getValue(
          $form_state->getUserInput(),
          array_merge($form['#parents'], [$elements['#field_name']])
        );
        $prev_key = NULL;
        foreach (array_keys($elements) as $key) {
          if (is_numeric($key)) {
            ...
            if ($prev_key !== NULL &&
                empty($field_input[$prev_key]['color'])) {
