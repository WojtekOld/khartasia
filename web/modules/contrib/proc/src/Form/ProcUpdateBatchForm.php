 ...
 */
class ProcUpdateBatchForm extends ProcUpdateForm {
  ...
  public function getUpdateCiphersData(): array {
    ...
    // Clear cache and reload the keyring entity to ensure fresh meta data.
    $keyringEntity = $keyring['keyring_entity'];
    $this->entityTypeManager->getStorage('proc')->resetCache([$keyringEntity->id()]);
    $keyringEntity = $this->entityTypeManager->getStorage('proc')->load($keyringEntity->id());

    $meta = $keyringEntity->get('meta')->getValue()[0] ?? [];
    if (empty($meta['update_jobs'])) {
      $this->logger->info('No update jobs found for user ID @userId.', ['@userId' => $this->currentUser->id()]);
      return [];
    }
    ...
    $this->messenger()->addStatus($this->t('There are @count content items that can be re-encrypted by you.', [
  ...
  private function applyMaxSizeLimit(array &$ciphers_data): void {
    $maxSize = $this->configFactory->get('proc.settings')
